...
plt.ylabel('U/bps')
marker_size = 5
plt.plot(np.linspace(0, loops_karman, loops_karman), Us_snc, linestyle='--', marker='+',
         markersize=marker_size, label='确定性不完全时钟同步模型')
plt.scatter([range(0, loops_karman)] * loops_mc, Us_mc, color='red', marker='.',
            alpha=0.9, label='蒙特卡洛随机实验')
plt.plot(np.linspace(0, loops_karman, loops_karman), Us_queue, linestyle='--', marker='.',
         markersize=marker_size, label='不完全时钟同步模型')
plt.plot(np.linspace(0, loops_karman, loops_karman), Us, linestyle='--', marker='^',
